...
=======
    ...
    if (declarator && declarator.init && ctx.isFunctionExpressionWithArgs(declarator.init)) {
        ...
        addRemoveInjectArray(d0.init.params, nr0, isSemicolonTerminated$0 ? nr1 : d0.init.range[1], d0.id.name);
    } else if (ctx.isFunctionDeclarationWithArgs(node)) {
        addRemoveInjectArray(node.params, nr0, nr1, node.id.name);
    } else if (node.type === "ExpressionStatement" && node.expression.type === "AssignmentExpression" &&
        ctx.isFunctionExpressionWithArgs(node.expression.right)) {
        var isSemicolonTerminated$1 = (ctx.src[nr1 - 1] === ";");
        var name = ctx.srcForRange(node.expression.left.range);
        addRemoveInjectArray(node.expression.right.params, nr0, isSemicolonTerminated$1 ? nr1 : node.expression.right.range[1], name);
    }
